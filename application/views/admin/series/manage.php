...
?>
	<ul class="pagination" style="margin-top: -5px">
		<?php
			...
			{
				echo '<li class="prev"><a href="' . site_url('admin/series/manage/'. 1) . '">&larr; Primo</a></li>';
			...
			if ($comics->paged->current_page < 5)
			{
				$page = 1;
				while ($page <= 7)
				{
					if ($page > $comics->paged->total_pages)
						break;
			...
			if ($comics->paged->current_page >= 5)
			{
				$last = (($comics->paged->current_page + 3) > $comics->paged->total_pages) ? $comics->paged->total_pages : $comics->paged->current_page + 3;
				for ($i = ($comics->paged->current_page - 3); $i <= $last; $i++)
				{
				if ($comics->paged->current_page == $i)
					echo '<li class="active"><a href="' . site_url('admin/series/manage/'.$i) .'">' . $i . '</a></li>';
				else
					echo '<li><a href="' . site_url('admin/series/manage/'.$i) .'">' . $i . '</a></li>';
				}
			}
			...
			if ($comics->paged->has_next)
			{
				...
				echo '<li class="prev"><a href="' . site_url('admin/series/manage/'. $comics->paged->total_pages) . '"> Ultimo &rarr;</a></li>';
			}
			else
